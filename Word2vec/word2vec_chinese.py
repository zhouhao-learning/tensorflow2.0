...
batch, labels = generate_batch(batch_size=2, num_skips=2, skip_window=1)
for i in range(2):
...
with graph.as_default():
    # Input data.
    train_inputs = tf.compat.v1.placeholder(tf.int32, shape=[None], name="x")
    train_labels = tf.compat.v1.placeholder(tf.int32, shape=[None, 1], name="y")
    ...
    with tf.device('/cpu:0'):
        ...
        embeddings = tf.Variable(
            tf.random.uniform([vocabulary_size, embedding_size], -1.0, 1.0), name="embedding")
        embed = tf.nn.embedding_lookup(params=embeddings, ids=train_inputs,name="embed")
        ...
        nce_biases = tf.Variable(tf.zeros([vocabulary_size]),dtype=tf.float32,name="nce_biases")
    ...
    optimizer = tf.compat.v1.train.GradientDescentOptimizer(1.0).minimize(loss,name="optimizer")
    ...
    for step in xrange(num_steps):
        ...
        if step % 10000 == 0:
            ...
            for i in xrange(valid_size):
                ...
                print(log_str)
    tf.compat.v1.train.Saver().save(session, ckpt_file_path,global_step=20)
...
def plot_with_labels(low_dim_embs, labels, filename='images/tsne.png',fonts=None):
    ...
    plt.figure(figsize=(18, 16))  # in inches
    for i, label in enumerate(labels):
        x, y = low_dim_embs[i, :]
        plt.scatter(x, y)
        plt.annotate(label, fontproperties=fonts, xy=(x, y), xytext=(5, 2),
